...
The following example clones the "[CDC echo application using emUSB-host](https://github.com/Infineon/mtb-example-psoc-edge-usb-host-cdc-echo)" application with the desired name "UsbHostDdcEcho" configured for the *KIT_PSE84_EVAL_EPC2* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSE84_EVAL_EPC2 --app-id mtb-example-psoc-edge-usb-host-cdc-echo --user-app-name UsbHostDdcEcho --target-dir "C:/mtb_projects"
   ```
